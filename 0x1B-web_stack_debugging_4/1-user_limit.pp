...
exec { 'update-file-limits-for-holberton':
  command => '/usr/bin/env bash -c "sed -i \'s/holberton hard nofile [0-9]*/holberton hard nofile 50000/\' /etc/security/limits.conf && sed -i \'s/holberton soft nofile [0-9]*/holberton soft nofile 50000/\' /etc/security/limits.conf"',
  path    => ['/usr/bin', '/bin'],
